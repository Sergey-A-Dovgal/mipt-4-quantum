...
\def \pd{\partial}
$$
   \left\{ i \hbar \dfrac{\pd}{\pd t} - \hat H \right\} \qu{\Psi(t)} = 0 \eqno (\ast)
$$
...
Полное решение УШ до включения возмущения можно записать в виде разложения по этой системе
$$
...
$$
    |C_n|^2 = w_n
$$
\item  Посмотрим на уравнение $(\ast)$. Пусть <<включено>> возмущение $\Hat V(t)$.
...
\sum_{k} C_k \left(
    ...
    \right)
$$
Для начала, учтём УШ нулевого (невозмущённого) порядка приближения. Из него:
$$
    i \hbar \sum_{k} \left\{
...
\sum_{k} C_k \left(
    ...
    \right)
$$
Умножим исходное равенство слева на $\uq{\Psi_{m}^{(0)}} (t)$, учтём ортонормированность системы.
$$
...
$$
    C_{nm}^{(1)} (T) = \dfrac{1}{i\hbar} \int_{0}^{T} V_{mn} (t') e^{i\omega_{mn} t'} dt'
$$
...
Говорить о стационарных состояних можно тогда, когда возмущение снято. Именно поэтому мы рассматриваем конечный промежуток времени. Промежутки времени должны быть не очень велики, и можно говорить о дискретнчых квантовых переходах. 

Вероятность перехода:
$$
    P_{nm} (T) = \dfrac{1}{h^2}\left|
\int_{0}^{T} V_{mn} (t') e^{i\omega_{mn} t'} dt'
    \right|^2
$$
...
\dfrac{1}{\hbar \omega_{mn}} \int_0^T \dfrac{d V_{mn} (t')}{dt'} e^{i \omega_{mn} t'} dt'
$$
...
$$
    \dfrac{d V_{mn}}{dt} \dfrac{1}{\omega_{mn}} \ll \hbar \omega_{mn} \eqno (\ast)
$$
$$
...
\right)
$$
Эта вероятность экспоненциально мала, $P_{nm} (T) \ll 1$. В первом приближении система остаётся в том же самом состоянии. Адиабатические изменения не приводят к изменению состояния.\footnote{Существует целая теория адиабатических возмущений. Она включена в задавальник.} 
...
$$
    \int_{-\infty}^{+\infty} f(\omega, T) d\omega = 2 \pi T = \textrm{const}
$$
...
$$
Итак, будем полагать, что время действия возмущения достаточно далеко.
\begin{eqnarray*}
...
$$
Получили <<золотое>> правило Ферми (полная вероятность перехода в единицу времени в группу состояния).
\end{itemize}
...
$$
$$
    C_{nm}^{(1)} (t) = \dfrac{1}{i \hbar} \int_0^T V_{mn}^{\pm} \underline{e^{\pm i \omega t}} e^{i \omega_{mn} t} dt
$$
...
$$
\begin{itemize}
  \item Если брать нижний знак, то новая энергия это старая энергия плюс $\hbar \omega$, система поглощает энергию.
$$
