...
\textbf{Вывод:}
$$
    \jpm \qlm = N_\pm \qu{\lam, \, \mu \pm 1} \eqno (\ast)
$$
...
$$
    ...
    \end{cases}
$$
Для того, чтобы реализовать эти условия, надо найти $N_\pm$. Воспользуемся $(\ast)$.
...
Заменим $\vec p$, $\vec r$ не операторы.
\begingroup
\def \nla {\nabla}
\def \hvr {\hat{\vec r}}
\def \hvp {\hat{\vec p}}
...
$$
    \qs{\phi}{m} = \Phi_m(\phi), \qquad -i \ud{}{\phi} = m \Phi_m(\phi)
$$
$$
...
$$
    \int_0^{2\pi} \Phi_m^\ast (\phi) \Phi_{m'} (\phi) d \phi = \dta_{mm'}
$$
Отсюда
$$
    C = \dfrac{1}{\sqrt{2\pi}}
